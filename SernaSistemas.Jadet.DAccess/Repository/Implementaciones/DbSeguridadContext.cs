...
namespace SernaSistemas.Jadet.DAccess.Repository
{
    public class DbSeguridadContext : IDbSeguridadContext
    {
        ...
        public DbSeguridadContext(BDJadetEntities entities)
        {
            contexto = entities;
        }
        public DbSeguridadContext(System.Data.Entity.DbContext dbContext)
        {
        ...
        public bool GuardarUsuario(ref UsuarioDTO usuario)
        {
            ...
            if (_rol != null)
            {
                Estatus _estatus = contexto.Estatus1.FirstOrDefault(e => e.Id == idEstatus);
                Usuario _usuario = contexto.Usuarios.FirstOrDefault(u => u.Id == id);
                if (_usuario == null)
                {
                ...
                _usuario.NomUsuario = usuario.NomUsuario;
        ...
        public EstatusesDTO ObtenerEstatuses(EstatusDTO estatus)
        {
            ...
            {
                resultado.AddRange(EstatusesDTO.ToDTO(contexto.Estatus1.ToList()));
            }
            return resultado;
        }
        ...
        public TiposEstatusDTO ObtenerTiposEstatus(TipoEstatusDTO tipo)
        {
            ...
            else if (!string.IsNullOrEmpty(tipo.Nombre))
            {
                resultado.AddRange(TiposEstatusDTO.ToDTO(
                    contexto.TipoEstatus1.Where(t => t.Nombre.ToUpper().Contains(tipo.Nombre.ToUpper())).ToList())
                    );
            }
            else
            {
